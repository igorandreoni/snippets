...
#NOTE: tweaked for DWF run June 2021
...
using Kowalski')
    ...
    # Query Kowalski
    info = query_kowalski_cone(ra, dec, search_rad, jd_start=date_start.jd, jd_end=date_end.jd,
                        programid_list=programid_list, drb_min=args.drb_min, ndethist_min=args.ndethist_min)

    info_clean = [] # Remove programid=3 outside a certain time window
    # Organize the results
    for alert in info:
        # Exclude programid=3 from before June 2 or after July 10, 2021
        if (alert["candidate"]["jd"] < 2459367 or alert["candidate"]["jd"] > 2459406) and alert["candidate"]["programid"] == 3:
            continue
        else:
            info_clean.append(alert)
    # Object IDs
    ...
    if args.out is None:
        outname = f"ztf_objects_ra{ra}_dec{dec}_r{search_rad}arcmin.csv"
    else:
        outname = args.out
    tab.write(outname, format='csv', overwrite=True)

    # Print and say goodbye
    print(f"Done! Created output file: {outname}")
